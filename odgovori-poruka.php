...
$query = "SELECT * FROM poruke WHERE id = $id";
$result = $conn->query($query);
$poruka = $result->fetch_assoc();

$conn->query("UPDATE poruke SET procitano = 1 WHERE id = $id");

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $conn->query("UPDATE poruke SET odgovoreno = 1 WHERE id = $id");
    header("Location: poruke-dash.php");
    exit();
}
...
<head>
    ...
    <title>Odgovori na poruku</title>
...
<body>
    ...
    <div class="content-dashboard">
        ...
        <h1 class="porukeHeader">POŠALJI ODGOVOR</h1>

        <div class="boxRespond">
            <div class="respondBox">
                <p><strong>Ime:</strong> <?= htmlspecialchars($poruka['ime']) ?></p>
                <p><strong>Email:</strong> <?= htmlspecialchars($poruka['email']) ?></p>
                <?php if (!empty($poruka['telefon'])): ?>
                    <p><strong>Broj:</strong> <?= htmlspecialchars($poruka['telefon']) ?></p>
                <?php endif; ?>
                <p><strong>Poruka:</strong> <?= htmlspecialchars($poruka['poruka']) ?></p>
                <form method="post" action="respondMail.php">
                    <input type="hidden" name="email" value="<?= htmlspecialchars($poruka['email']) ?>">
                    <input type="hidden" name="id" value="<?= htmlspecialchars($poruka['id']) ?>">
                    <textarea name="odgovor" class="inputOdgovor"><?= isset($_SESSION['old_input']['odgovor']) ? htmlspecialchars($_SESSION['old_input']['odgovor']) : '' ?></textarea>
                    <div class="sndButtonOdg">
                        <button type="submit" class="submit-button">Pošalji odgovor</button>
                    </div>
                </form>
            </div>
        </div>
...
<?php if (isset($_SESSION['error'])): ?>
    ...
    <?php unset($_SESSION['error'], $_SESSION['old_input']); ?>
<?php endif; ?>
...
<script>
    ...
    form.addEventListener("submit", function (e) {
        const textarea = form.querySelector("textarea[name='odgovor']");
        const odgovor = textarea.value.trim();

        if (!odgovor) {
            e.preventDefault();
            notyf.open({
                type: 'warning',
                message: 'Molimo unesite odgovor prije slanja.'
            });
        }
    });
</script>
