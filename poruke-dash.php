...
$filter = isset($_GET['filter']) ? $_GET['filter'] : 'sve';
$where_clause = "";
switch ($filter) {
    case 'procitane':
        $where_clause = "WHERE procitano = 1"; 
        break;
    case 'neprocitane':
        $where_clause = "WHERE procitano = 0";
        break;
    case 'odgovorene':
        $where_clause = "WHERE odgovoreno = 1";
        break;
    case 'neodgovorene':
        $where_clause = "WHERE odgovoreno = 0";
        break;
    default:
        $where_clause = "";
}

$poruke_query = "SELECT * FROM poruke $where_clause ORDER BY datum DESC";
$poruke_result = $conn->query($poruke_query);
...
<body>
    ...
    <!--Navbar fixed left-->
        <div class="navbar-fixed-left">
            ...
            Blog</a>
            <a href="poruke-dash.php" class="svg-link active">
                <svg xmlns="http://www.w3.org/2000/svg" width="800px" height="800px" viewBox="0 0 24 24" fill="none">
            ...
            Recenzije</a>
            <a href="analitika-dash.php" class="svg-link">
    ...
    <!-- Page content -->
        <div class="content-dashboard">
            <h1 class="porukeHeader">PORUKE</h1>

            <div class="filter-container">
                <div class="searchMsg">
                    <svg class="iconMsg" aria-hidden="true" viewBox="0 0 24 24"><g><path d="M21.53 20.47l-3.66-3.66C19.195 15.24 20 13.214 20 11c0-4.97-4.03-9-9-9s-9 4.03-9 9 4.03 9 9 9c2.215 0 4.24-.804 5.808-2.13l3.66 3.66c.147.146.34.22.53.22s.385-.073.53-.22c.295-.293.295-.767.002-1.06zM3.5 11c0-4.135 3.365-7.5 7.5-7.5s7.5 3.365 7.5 7.5-3.365 7.5-7.5 7.5-7.5-3.365-7.5-7.5z"></path></g></svg>
                    <input placeholder="Search" type="search" class="inputSearchMsg" id="searchInput">
                </div>

                <div class="filter-buttons">
                    <a href="poruke-dash.php?filter=sve" class="filter-btn <?= $filter == 'sve' ? 'active' : '' ?>">Sve</a>
                    <a href="poruke-dash.php?filter=procitane" class="filter-btn <?= $filter == 'procitane' ? 'active' : '' ?>">Pročitane</a>
                    <a href="poruke-dash.php?filter=neprocitane" class="filter-btn <?= $filter == 'neprocitane' ? 'active' : '' ?>">Nepročitane</a>
                    <a href="poruke-dash.php?filter=odgovorene" class="filter-btn <?= $filter == 'odgovorene' ? 'active' : '' ?>">Odgovorene</a>
                    <a href="poruke-dash.php?filter=neodgovorene" class="filter-btn <?= $filter == 'neodgovorene' ? 'active' : '' ?>">Neodgovorene</a>
                </div>
            </div>
            ...
            <div class="table-container">
                <table class="messages-table" id="messagesTable">
                    ...
                    <tbody>
                        <?php if ($poruke_result->num_rows > 0): ?>
                            <?php while ($poruka = $poruke_result->fetch_assoc()): ?>
                                <tr class="<?= $poruka['procitano'] ? '' : 'highlight' ?>">
                                    <td><?= htmlspecialchars($poruka['ime']) ?></td>
                                    <td><?= htmlspecialchars($poruka['email']) ?></td>
                                    <td><?= htmlspecialchars(substr($poruka['poruka'], 0, 30)) . (strlen($poruka['poruka']) > 30 ? '...' : '') ?></td>
                                    <td><?= date('d.m.Y H:i', strtotime($poruka['datum'])) ?></td>
                                    <td><?= $poruka['procitano'] ? 'Pročitano' : 'Nepročitano' ?> / <?= $poruka['odgovoreno'] ? 'Odgovoreno' : 'Neodgovoreno' ?></td>
                                    <td><a href="odgovori-poruka.php?id=<?= $poruka['id'] ?>" class="reply-link">Odgovori</a></td>
                                    <td>
                                        <a href="#" class="delete-link" onclick="openDeleteModal(<?= $poruka['id'] ?>); return false;">
                            ...
                            <tr>
                                <td colspan="7">
                                    <div class="no-messages-wrapper">
                                        <span class="no-messages">Nema poruka za prikaz</span>
                                    </div>
                                </td>
                            </tr>
                        <?php endif; ?>
                ...
                <!--popup za delete-->
                    <div id="deleteModal" class="modal-overlay hidden">
                        <div class="modal-box">
                            ...
                            <p id="deleteMessage">Da li ste sigurni da želite izbrisati ovu poruku?</p>
                            <form method="post" action="obrisi-poruku.php">
...
<script>
    ...
    document.getElementById('deleteMessage').innerText = `Da li ste sigurni da želite izbrisati izabranu poruku?`;
    ...
    if (params.get("deleted") === "1") {
        notyf.open({
            type: 'success',
            message: 'Poruka je izbrisana!'
        });
    }
</script>

<?php if (isset($_GET['msg']) && $_GET['msg'] === 'odgovor-poslan'): ?>
    <script>
    ...
    const notyf = new Notyf({
        ...
        notyf.open({
            type: 'success',
            message: 'Odgovor je uspješno poslan!'
        });
