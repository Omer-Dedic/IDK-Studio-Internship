...
if (isset($_POST['review_id'])) {
    $delete_id = intval($_POST['review_id']);
    $delete_sql = "DELETE FROM recenzija WHERE id = ?";
...
$sql = "SELECT id, Ime, Prezime, Recenzija, Ocjena, vrijeme_recenzije FROM recenzija ORDER BY vrijeme_recenzije DESC";
...
if ($total_reviews > 0) {
    ...
    $average_rating = number_format($sum / $total_reviews, 1); // Format to 1 decimal place
    $result->data_seek(0); // Reset pointer again for the main loop
}
?>
<!DOCTYPE html>
<head>
    ...
    <title>Dashboard-Recenzije</title>
</head>
<body>
    ...
    <!--Navbar fixed left-->
        <div class="navbar-fixed-left">
            ...
            Poruke</a>
            <a href="rec-dash.php" class="svg-link active">
    ...
    <!--Page content-->
        <div class="content-dashboard-rec">
            <div class="cnt-te">
            <div class="header-row">
                <h1 class="recHead">Recenzije</h1>
                <div class="average-rating">
                    <?= $average_rating ? $average_rating : 'N/A' ?><span class="star">★</span>
                </div>
            </div>
                ...
                <?php while ($row = $result->fetch_assoc()): ?>
                    ...
                    <div class="review-card">
                        <div class="review-header">
                            <span class="reviewer-name"><?= htmlspecialchars($row['Ime'] . ' ' . $row['Prezime']) ?></span>
                            <span class="review-date"><?= date('d M Y', strtotime($row['vrijeme_recenzije'])) ?></span>
                            <div class="rating-container">
                                <span class="star-rating">
                                    <?php
                                    $rating = intval($row['Ocjena']);
                                    for ($i = 1; $i <= 5; $i++) {
                                        echo $i <= $rating ? '★' : '☆';
                                    }
                                    ?>
                                </span>
                                <a href="#" class="delete-btn" onclick="openDeleteModal(<?= $row['id'] ?>); return false;">
                                    <span class="material-icons">delete</span>
                                </a>
                            </div>
                        </div>
                        <p class="review-text">
                            <?=  nl2br(strip_tags($row['Recenzija'])) ?>
                        </p>
                    </div>
                <?php endwhile; ?>
            <?php else: ?>
                <p class="narec">Nema dostupnih recenzija</p>
            <?php endif; ?>
            ...
            <div id="deleteModal" class="modal-overlay hidden">
                <div class="modal-box">
                    ...
                    <p id="deleteMessage">Da li ste sigurni da želite izbrisati ovu recenziju?</p>
                    <form method="POST" action="rec-dash.php">
    ...
    <script>
        function openDeleteModal(reviewId) {
            ...
            document.getElementById('deleteMessage').innerText = `Da li ste sigurni da želite izbrisati ovu recenziju?`;
